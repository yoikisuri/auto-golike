...
class SnapChat:
    ...
    async def job(self):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.request("GET", url, params=params, headers=self.headers) as response:
                return await response.json()
    ...
    async def complete(self, ads_id):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.request("POST", url, headers=self.headers, json=data) as response:
                return await response.json()
    ...
    async def run(self):
        ...
        for index in range(5):
            ...
            if job["status"] == 200:
                ...
                print(f"[{index}/{count}][{time.time()}][THÀNH CÔNG][+{complete['data']['prices']}]") 
